...
<div class="container">
    <div class="row justify-content-center">
       ...
       <!-- employee_id Field -->           
        ...
        $emees = DB::table('employees as e')
                            ->select(DB::raw('e.first_name name, e.id id, u.employee_id, u.id'))
                            ->leftJoin('users as u', 'u.employee_id', '=', 'e.id')
                            ->where('u.employee_id', null)
                             ->whereNotIn('u.employee_id', [1, 2, 3])
                            ->pluck('name', 'id')
                            ;  

        $employees = Employee::select(
                            DB::raw("CONCAT(first_name,' ',last_name) AS name"),'employees.id')
                            ->leftJoin('users as u', 'u.employee_id', '=', 'employees.id')
                            ->where('u.employee_id',null)
